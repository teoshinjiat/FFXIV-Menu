...
global menuData := []
menuData[ 1 ] := {function:"gAutoSynthesis", value:"vMainScript1", label:"Auto Synthesis", subOptionGuiType:"Checkbox", subOptionGuiStyle:"x60", subOptions:[{value:"Disabled vMainScript1_SubCheckBox1", label:"Auto refresh food"}, {value:"Disabled vMainScript1_SubCheckBox2", label:"Auto refresh medicine"}, {value:"Disabled vMainScript1_SubCheckBox3", label:"Collectable"}]}
...
menuData[ 5 ] := {function:"gEulmore", value:"vMainScript5", label:"Auto Eulmore Turnin", subOptionGuiType:"ListBox", subOptionGuiStyle:"w350", subOptions:[{value:"Disabled vMainScript5_SubCheckBox1", label:"Craftsman's Command Materia X"}]}
...
for i, obj in menuData{ ;mainBox mainscript boxes
	...
	for key, field in menuData[i].subOptions{ ;subBox options(parameters)
		...
		DebugWindow("adding gui..",0,1,200,0)
...
uncheckMainScriptBoxes(selectedIndex){
	for i, obj in menuData{
		...
		{ 
			...
			;for key, field in menuData[i].subOptions{ ;looping sub boxes
			Loop, 3 { ; HARDCODED, to uncheck and disable other subBoxes
				GuiControl, Disabled, MainScript%mainScriptLoopIndex%_SubCheckBox%A_Index%
				if(menuData[i].subOptionGuiType="Checkbox"){
					GuiControl,, MainScript%mainScriptLoopIndex%_SubCheckBox%A_Index%, 0 ;to uncheck other subBoxes when clicking other mainBox away from current selected mainBox				
				} else { ; deselect item from selected items in List
					GuiControl, Choose, MainScript%mainScriptLoopIndex%_SubCheckBox%A_Index%, 0
				}
			}
		} 
	}
