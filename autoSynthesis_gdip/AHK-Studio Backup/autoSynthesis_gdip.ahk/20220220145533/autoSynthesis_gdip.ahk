...
Loop{	
	...
	{
		processWindowImage()
		ErrorLevel:=searchImage("still-crafting")
	}
...
preHealthCheck(){
	...
	buttonToPress:=""
	ErrorLevel:=searchImage("spritbond_full")
	if(ErrorLevel=0){
		DebugWindow("will extract materia next",0,1,2000,0)		
		buttonToPress:=3
	}
	
	ErrorLevel:=searchImage("durability_yellow")
	if(ErrorLevel=0){
	...
	if(eatFoodFlag=1){
		ErrorLevel:=searchImage("food_refresh")
		if(ErrorLevel=0){
	...
	if(eatMedicineFlag=1){
		ErrorLevel:=searchImage("medicine_refresh")
		if(ErrorLevel=0){
...
durabilityCheck(){
	...
	{
		processWindowImage() ;save current window image
		ErrorLevel:=searchImage("repair_button")
	}
	if(ErrorLevel=0){
...
spiritbondCheck(){
	...
	While (ErrorLevel = 0)
	{
		processWindowImage() ;save current window image
		sleep, 1000
		ErrorLevel:=searchImage("spiritbond_100percent")
		if(ErrorLevel=0){
			ControlSend, , {Right}, ahk_class FFXIVGAME
			sleep, 1000
			ControlSend, , ``, ahk_class FFXIVGAME
...
autoSynthesis() {
	...
	{
		processWindowImage() ;save current window image
		ErrorLevel:=searchImage("synthesis_button")
		if(ErrorLevel=1){
...
detectDurability(){
	...
	durability := ""
	while(durability=""){
		DebugWindow("looking for dura",0,1,1,0)		
		errorLevel:=searchImage(35)
		if(errorLevel>0){
			log("35 detected")
			durability="35"
		} else{
			errorLevel:=searchImage(40)
			if(errorLevel>0){
				log("40 detected")
				durability="40"
			} else{
				errorLevel:=searchImage(70)
				if(errorLevel>0){
					log("70 detected")
					durability="70"
				} else {
					errorLevel:=searchImage(80)
					if(errorLevel>0){
						log("80 detected")
						durability="80"
					}
...
searchImage(filename){
	DebugWindow("searchImage()",0,1,1,0)
	...
	Gdip_DisposeImage(ppBitMap)						;delete as is not longer used
	return result
}
...
processWindowImage(){
	DebugWindow("processWindowImage() start",0,1,1,0)
	GDIP_DisposeImage(ppBitMap)
	
	if !ppBitMap := Gdip_BitmapFromHWND(gameId)
	{
		log("Gdip_BitmapFromHWND fail")
		ExitApp
	}
	
	if Gdip_SaveBitmapToFile(ppBitMap, img := A_ScriptDir "\screen.png")
	{
		log("Gdip_SaveBitmapToFile fail11")
		ExitApp
	}
	haystack:=Gdip_CreateBitmapFromFile("screen.png")
	DebugWindow("processWindowImage() end",0,1,1,0)
