...
searchDurability(filename){
	...
	needlePath=filename.png
	bmpNeedle := Gdip_CreateBitmapFromFile(needlePath)
	
	result:= Gdip_ImageSearch(bmpHaystack,bmpNeedle,LIST,,,,,80,0,1,0)
...
saveCurrentScreenImage(){
	if Gdip_SaveBitmapToFile(pBitmap, img := "autoSynthesis_gdip\screen.png")
	{
		log("Gdip_SaveBitmapToFile fail")
		ExitApp
	}
	bmpHaystack := Gdip_CreateBitmapFromFile("screen.png")
}


if Gdip_SaveBitmapToFile(pBitmap, img := A_ScriptDir "\gdip.png")
{
	log("Gdip_SaveBitmapToFile fail")
	ExitApp
}
